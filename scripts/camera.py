    ...
    def render(self, surf, walls, players):
        ...
        for entity in sorted(self.entities, key = lambda e: e[1][1]):
            if entity in self.walls:
                if self.game.render_scroll[0] // self.game.tilemap.tile_size < entity[0]['pos'][0] and ((self.game.render_scroll[0] + self.game.display.get_width()) // self.game.tilemap.tile_size + 1) > entity[0]['pos'][0] and self.game.render_scroll[1] // self.game.tilemap.tile_size < entity[0]['pos'][1] and ((self.game.render_scroll[1] + self.game.display.get_height()) // self.game.tilemap.tile_size + 1) > entity[0]['pos'][1]:
